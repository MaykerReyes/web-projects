(function () {
  ...
  const elements = {
    ...
    views: {
      ...
      streaks: document.getElementById("streaks-view")
    },
    ...
    cbAttempts: document.getElementById("cb-attempts"),
    cbHistory: document.getElementById("cb-history"),
    cbDigits: [
      document.getElementById("cb-d1"),
      document.getElementById("cb-d2"),
      document.getElementById("cb-d3"),
      document.getElementById("cb-d4")
    ],
    cbFeedback: document.getElementById("cb-feedback"),
    cbGuessBtn: document.getElementById("cb-guess-btn"),
    cbStartBtn: document.getElementById("cb-start-btn"),
  ...
  const lightsState = {
    ...
    inputIndex: 0,
    round: 0,
    bestRound: Number(localStorage.getItem("studyQuestLightsBest")) || 0,
    accepting: false,
    patternDelay: 850,
    playToken: 0
  };
  ...
  function renderStreaks() {
    if (!elements.currentStreakVal) return;
    elements.currentStreakVal.textContent = state.streak;
    elements.bestStreakVal.textContent = state.bestStreak || 0;
    ...
    if (!state.loginDates.includes(today)) {
      ...
      if (state.loginDates.length > 30) state.loginDates.shift();
    }
    ...
    for(let i=29; i>=0; i--) {
      ...
      past30Days.push(`${d.getFullYear()}-${d.getMonth()+1}-${d.getDate()}`);
    }

    past30Days.forEach(dateStr => {
      const dayEl = document.createElement("div");
      dayEl.className = `heatmap-day ${state.loginDates.includes(dateStr) ? "active" : ""}`;
      dayEl.title = dateStr;
      elements.heatmapContainer.appendChild(dayEl);
    });
  }
  ...
  function startCbGame() {
    cbState.running = true;
    cbState.attempts = 0;
    cbState.secret = [];
    for (let i = 0; i < 4; i++) {
      cbState.secret.push(Math.floor(Math.random() * 6) + 1);
    }
    elements.cbAttempts.textContent = "0";
    elements.cbHistory.innerHTML = "";
    elements.cbFeedback.textContent = "Guess the 4-digit code (1-6).";
    elements.cbDigits.forEach(d => {
      d.value = "";
      d.disabled = false;
    });
  }

  function handleCbGuess() {
    if (!cbState.running) return;
    const guess = elements.cbDigits.map(d => parseInt(d.value, 10));
    if (guess.some(isNaN) || guess.some(v => v < 1 || v > 6)) {
      elements.cbFeedback.textContent = "Enter numbers 1-6 in all boxes.";
      return;
    }
    
    cbState.attempts++;
    elements.cbAttempts.textContent = cbState.attempts;
    
    let exact = 0;
    let partial = 0;
    const secretCopy = [...cbState.secret];
    ...
    guess.forEach(g => {
      ...
      d.className = "cb-history-digit";
      d.textContent = g;
      row.appendChild(d);
    });
    const pegs = document.createElement("div");
    pegs.className = "cb-history-pegs";
    pegs.textContent = "🟩".repeat(exact) + "🟨".repeat(partial);
    row.appendChild(pegs);
    elements.cbHistory.prepend(row);
    
    if (exact === 4) {
      elements.cbFeedback.textContent = `You cracked it in ${cbState.attempts} tries! +50 XP.`;
      awardXp(50);
      cbState.running = false;
      elements.cbDigits.forEach(d => d.disabled = true);
    } else if (cbState.attempts >= cbState.maxAttempts) {
      elements.cbFeedback.textContent = `Out of tries. The code was ${cbState.secret.join("")}.`;
      cbState.running = false;
      elements.cbDigits.forEach(d => d.disabled = true);
    } else {
      elements.cbFeedback.textContent = "Try again.";
    }
    
    elements.cbDigits.forEach(d => d.value = "");
    elements.cbDigits[0].focus();
  }
  ...
  if (elements.cbStartBtn) {
    elements.cbStartBtn.addEventListener("click", startCbGame);
    elements.cbGuessBtn.addEventListener("click", handleCbGuess);
  }
